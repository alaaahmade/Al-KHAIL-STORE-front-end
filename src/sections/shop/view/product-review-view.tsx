...
import { Box, Card, Typography, Button, Avatar, LinearProgress, Stack, Pagination, MenuItem, Select, FormControl, InputLabel, Chip, CircularProgress, Alert, ListItemText, TextField, IconButton } from '@mui/material';
...
export default function ProductReviewView({ productId }: { productId: string }) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          // @ts-ignore
          import('react-toastify').then(({ toast }) => toast.error(`Failed to upload file: ${file.name}`));
        }
      }
    }
  ...
  const [selectedSort, setSelectedSort] = useState<'recent'|'oldest'|'highest'|'lowest'>('recent');
  ...
  const pagedReviews = filteredReviews.slice((currentPage-1)*reviewsPerPage, currentPage*reviewsPerPage);
    ...
    <Box sx={{ p: { xs: 1, md: 4 }, background: '#fafbfc', minHeight: '100vh' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="body2" color="grey.600">
          <span onClick={() => router.push('/shop/products')} style={{ cursor: 'pointer', color: '#888' }}>Back to Products</span> &nbsp; &gt; &nbsp; {reviews[0]?.product?.category[0]?.categoryName || 'review'} &nbsp; &gt; &nbsp;
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 2 }}>
            <FormControl size="small" sx={{ minWidth: 120 }}>
              ...
              <Select labelId="sort-label" label="Sort By" value={selectedSort} onChange={e => { setSelectedSort(e.target.value as any); setCurrentPage(1); }}>
            ...
            <FormControl size="small" sx={{ minWidth: 120 }}>
              ...
              <Select labelId="star-label" label="Stars" value={selectedStar} onChange={e => { setSelectedStar(e.target.value as any); setCurrentPage(1); }}>
            ...
            <FormControl size="small" sx={{ minWidth: 120 }}>
              ...
              <Select labelId="perpage-label" label="Per Page" value={reviewsPerPage} onChange={e => { setReviewsPerPage(Number(e.target.value)); setCurrentPage(1); }}>
            ...
            {pagedReviews?.length > 0 ? 
             pagedReviews.map((review) => (
              <Card key={review.id} sx={{ p: 3 }}>
                              ...
                              InputProps={{
                                endAdornment: (
                                  <IconButton onClick={() => handleRemoveReplyFile(idx)}>
                                    <span style={{ color: '#d32f2f', fontWeight: 600, fontSize: 18 }}>×</span>
                                  </IconButton>
                                ),
                              }}
                            />
                ...
                <Button onClick={() => setDisplayReplyInput(review.id)} size='small'>Reply</Button>
              </Card>
            ))
            :
