    ...
    <Box sx={{ background: '#f7f8fa', minHeight: '100vh', py: 4 }}>
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={3} alignItems="flex-start">
          {/* Left: Cart */}
          <Box flex={2}>
            <Typography variant="h5" sx={{ mb: 2, fontFamily: 'serif', fontWeight: 500 }}>
              Shopping Cart
            </Typography>
                ...
                cart?.items?.map((item: any, idx: number) => (
                  ...
                  >
                    ...
                    <Box flex={1} minWidth={0}>
                      <Typography variant="subtitle2" sx={{ fontWeight: 600, fontFamily: 'serif' }}>
                        {item.name}
                      </Typography>
                      ...
                      <Stack direction="row" alignItems="center" spacing={1} sx={{ mt: 1 }}>
                        <IconButton onClick={() => updateQuantity(item)} size="small">
                        ...
                        <IconButton
                          size="small"
                          onClick={() => addToCart(item.product.id, item.product.standardPrice)}
                        >
                          <Icon icon="ic:round-add" />
                        </IconButton>
                      </Stack>
                    </Box>
                    <Typography sx={{ minWidth: 64, textAlign: 'right', fontWeight: 500 }}>
                      ${(+item?.product?.standardPrice)?.toFixed(2)}
                    </Typography>
