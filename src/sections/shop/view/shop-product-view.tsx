    ...
    <Box>
      <Stack direction="row" alignItems="center" spacing={1} justifyContent={'space-between'}>
        ...
        <Stack direction="row" spacing={1} alignItems="center"> 
          <Rating value={product?.comments?.length > 0 ? product.comments.reduce((sum: number, r: any) => sum + r.rating, 0) / product.comments.length : 0} readOnly precision={0.5} size="small" sx={{ color: '#FFD700' }} />
      ...
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mt: 2 }}>
        ...
        <Box>
          <Typography variant="body2" sx={{ fontWeight: 700 }}>{`Sold by ${product?.store?.name}`}</Typography>
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            <Typography variant="caption" color="text.secondary">
              ...
              {product?.store?.seller?.user?.firstName ? product.store.seller.user.firstName : 'Seller'}
            </Typography>
            <Typography variant="caption" color="text.secondary">
              (Seller)
            </Typography>
...
export default function ShopProductView() {
  ...
  if(!currentProduct) return <Typography variant="h6" sx={{textAlign:'center', mt:8}}>Product Not Found</Typography>
    ...
    <Box sx={{ background: '#f7f8fa', minHeight: '100vh', py: 0 }}>
      <Box sx={{ maxWidth: 1100, mx: 'auto', background: '#fff', borderRadius: 4, p: 1}}>
        <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: 'block' }}>
          Home &gt; {currentProduct?.category?.[0]?.categoryName || 'Category'} &gt; {currentProduct?.productName}
        </Typography>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={4}>
          <ProductImageGallery images={[...currentProduct?.productGallery, currentProduct?.productImage] || []} mainImage={currentProduct?.productImage} />
