...
export function ProductTabs({ product }: { product: any }) {
  ...
  const initialTab = tabParam && !isNaN(Number(tabParam)) && Number(tabParam) < tabLabels.length ? Number(tabParam) : 0;
    ...
    <Box sx={{ mt: 4 }}>
      ...
      <Box sx={{ minHeight: 80 }}>
        ...
        <CustomerReviews
          ...
          reviewSummary={{
            average: product?.comments?.length > 0 ? (product.comments.reduce((sum: number, r: any) => sum + r.rating, 0) / product.comments.length).toFixed(1) : 0,
            total: product?.comments?.length || 0,
            breakdown: [
              { stars: 5, percent: Math.round((product?.comments?.filter((r: any) => r.rating === 5).length || 0) / (product?.comments?.length || 1) * 100) },
              { stars: 4, percent: Math.round((product?.comments?.filter((r: any) => r.rating === 4).length || 0) / (product?.comments?.length || 1) * 100) },
              { stars: 3, percent: Math.round((product?.comments?.filter((r: any) => r.rating === 3).length || 0) / (product?.comments?.length || 1) * 100) },
              { stars: 2, percent: Math.round((product?.comments?.filter((r: any) => r.rating === 2).length || 0) / (product?.comments?.length || 1) * 100) },
              { stars: 1, percent: Math.round((product?.comments?.filter((r: any) => r.rating === 1).length || 0) / (product?.comments?.length || 1) * 100) },
            ],
