    ...
    <Stack direction={'row'}  spacing={3} justifyContent="center" >
      {testimonials.map((t) => (
        <Grid item xs={12} md={3.5} key={t.name} sx={{ background: '#FFF6FB'}}>
          <Stack direction={'column'} alignItems={'flex-start'} gap={2} justifyContent={'center'} sx={{ p: 3, borderRadius: 3, textAlign: 'left' }}>
            ...
            <Typography sx={{lineHeight: 2}} variant="body2" color="text.secondary" mb={2}>{t.text}</Typography>
          </Stack>
        </Grid>
      ))}
    </Stack>
