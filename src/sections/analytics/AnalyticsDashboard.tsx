    ...
    const statsData = [
      {
        ...
        positive: analytics?.revenueChange ? (analytics.revenueChange.toString().startsWith('+') ? 'Yes' : 'No') : '-',
      },
      {
        label: 'Orders',
        value: analytics?.orderCount !== undefined ? analytics.orderCount.toLocaleString() : '-',
        change: analytics?.orderCountChange || '-',
        positive: analytics?.orderCountChange ? (analytics.orderCountChange.toString().startsWith('+') ? 'Yes' : 'No') : '-',
      },
      {
        label: 'Average Order Value',
        value: analytics?.avgOrderValue !== undefined ? analytics.avgOrderValue.toLocaleString() : '-',
        change: analytics?.avgOrderValueChange || '-',
        positive: analytics?.avgOrderValueChange ? (analytics.avgOrderValueChange.toString().startsWith('+') ? 'Yes' : 'No') : '-',
      },
      {
        label: 'Conversion Rate',
        value: analytics?.conversionRate !== undefined ? `${analytics.conversionRate}%` : '-',
        change: analytics?.conversionRateChange || '-',
        positive: analytics?.conversionRateChange ? (analytics.conversionRateChange.toString().startsWith('+') ? 'Yes' : 'No') : '-',
      },
    ];
      ...
      <Card sx={{ p: 3 }}>
        ...
        <Grid container spacing={3}>
              ...
              {analytics && analytics.topProducts && 
                ...
                <ListItem sx={{mb: 2}} key={prod.id}>
                  ...
                  sx={{backgroundColor: theme.palette.grey[50]}}
                    ...
                    secondary={<Typography sx={{fontSize: 13, ml: 3}} variant="body2" color="text.secondary">{prod.unitsSold } units sold</Typography>}
                  />
          ...
          <Grid item xs={12} md={6}>
            ...
            {analytics && analytics.salesByCategory && analytics.salesByCategory.length > 0 &&
